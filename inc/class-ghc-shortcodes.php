	...
	private function get_locations_buttons( $style = 'link' ) : string {
		...
		foreach ( $this->get_conventions_info() as $convention ) {
			if ( 'link' === $style ) {
				...
				$content .= '<input type="' . esc_attr( $style ) . '" class="convention filter" name="convention" id="' . esc_attr( $convention['slug'] ) . '" required>
				<label for="' . esc_attr( $convention['slug'] ) . '" class="convention filter button hollow">' . wp_kses_post( $convention['convention_short_name'] ) . '</label> ';
			}
		}
		return $content;
	}
